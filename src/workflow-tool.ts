    ...
    script: Type.Optional(
      Type.String({
        description: [
          ...
          "多阶段工作默认用 pipeline——屏障（用 parallel() 等齐所有前序结果）只在确实需要跨条目上下文时才正确：对全量结果去重/合并后再做昂贵下游、总量为零时提前退出、或后一阶段的提示需要引用「其他发现」做比较。「概念上分开」或「代码更整洁」不构成屏障理由——flatten/map/filter 放进 stage 内即可。拿不准时用 pipeline。",
          "调用前先做内联侦察（列出文件、找到通道、划定 diff 范围）得出工作清单，再让工作流扇出处理——只需要在编排前知道清单的形状，不必在任务前。",
          "常见单阶段工作流（理解/设计/审查/研究/迁移）可按轮串联成更大的工作：每轮先读结果再决定下一阶段，你始终在环内。",
          "agent() 可能返回 null——用户中途跳过，或子代理在重试后仍死于终态错误；消费结果前先 filter(Boolean)，并把 null 当缺失覆盖处理。",
          "子代理被告知最终文本就是返回值——它们返回原始数据而非面向人的话术；需要结构化结果时用 schema 选项。",
          "给同一主张配备不同视角的怀疑者（perspective-diverse）优于同质重复的验证者。当工作流限制了覆盖（top-N、不重试、抽样）时，用 log() 声明丢弃了什么——静默截断会被读成「已全覆盖」。「彻底审查/全面审计」类请求往更全面的规模配，「快速检查」从简。",
          "meta 必须是纯字面量——无变量、函数调用、展开或模板插值。",
          "诊断已完成工作流的空结果或意外结果前，先读该运行的 journal（记录每个 agent 的实际返回值），不要假设缓存结果非空。",
          "子代理可调用会话已连接的 MCP 工具——schemas 按需加载；交互式鉴权的服务器在无头/定时运行中可能缺失。",
        ].join(" "),
      }),
    ),
    name: Type.Optional(
