...
/**
 * Create an isolated worktree under `<repoRoot>/.omp/worktrees/<name>` on
 * branch `omp/wf/<name>`. The deterministic name keeps resume keys stable.
 */
export async function createWorktree(baseCwd: string, name: string): Promise<Worktree> {
  ...
  const path = join(repoRoot, ".omp", "worktrees", id);
  const branch = `omp/wf/${id}`;
  try {
