...
import { type ExtensionCommandContext, type Theme } from "./omp-api.js";
import {
  Container,
  type SelectItem,
  SelectList,
...
import { createSelectListTheme } from "./omp-theme.js";
...
/** `/workflows-models` implementation, loaded on first invocation. */
export async function runWorkflowModelsCommand(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const saved = loadModelTierConfig();
  let config = saved ?? buildDefaultTierConfig(currentModel, listAvailableModels(ctx.modelRegistry));
  // With no model-tiers.json on disk, the tiers shown are an unsaved
  // in-memory suggestion — every agent() with opts.tier still falls back to
  // the session model until they're written. Start dirty so the menu offers
  // "Save and exit" instead of stranding the user on a plain "Exit" that
  // silently discards the very defaults they just reviewed.
  let dirty = saved === null;
  ...
  while (true) {
    ...
    // Every entry is selectable: the host's selector has no separator
    // concept and starts the cursor on index 0, so a decorative rule would
    // read as a blank, dead first option — picking it matches no branch
    // below and just redraws the menu.
    const menuOptions = tiers.map((name) => `${name} tier → ${config.tiers[name]}`);
...
export async function editSingleTier(
  ...
  const selectedModel = await ctx.ui.custom<string | null>((tui: TUI, theme: Theme, _keybindings, done) => {
    ...
    const selectTheme: SelectListTheme = createSelectListTheme(theme);
