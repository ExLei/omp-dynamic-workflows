...
import type { TSchema } from "./omp-typebox.js";
...
export interface WorkflowRunOptions extends WorkflowAgentOptions {
   ...
   * run for determinism. Defaults to scanning `.omp/agents` (project) +
   * `~/.omp/agent/agents` (user, primary) + `~/.omp/agents` (user, deprecated
  ...
  /**
   * Live cumulative usage for a still-running agent (throttled). Fired only for
   * display: the run's own accounting (shared.spent, the token budget, the
   * persisted aggregate) is still settled once at onAgentEnd. Each event carries
   * the agent's usage SO FAR — replace the previous value, never add to it.
   */
  onAgentUsage?: (event: { id: string; label: string; phase?: string; tokenUsage: AgentUsage }) => void;
  onTokenUsage?: (usage: {
    input: number;
    output: number;
    total: number;
...
export interface AgentOptions<TSchemaDef extends TSchema | undefined = TSchema | undefined> {
  ...
  isolation?: "worktree";
  /**
   * Name of a registered subagent definition (`.omp/agents/<name>.md`, project >
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          ...
          try {
            ...
            const runPromise = agentRunner.run(prompt, {
              ...
              // Display-only: never folded into shared.tokenUsage/spent here —
              // recordTokens() still settles this agent's real spend once, from
              // the terminal onUsage above.
              onUsageProgress: options.onAgentUsage
                ? (u: AgentUsage) => {
                    options.onAgentUsage?.({ id: deltaKey, label, phase: assignedPhase, tokenUsage: u });
                  }
                : undefined,
