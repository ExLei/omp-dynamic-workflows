  ...
  runs(): RunRow[] {
    return this.persistedRuns().map((p) => {
      ...
      return {
        ...
        // Live per-agent cost until the run finalizes its own aggregate.
        cost: Math.max(fromAgents.cost, usage?.cost ?? 0),
      };
    });
  }
...
export class NavigatorState {
  ...
  pagerOpen = false;
  /**
   * A modal choice layered over the current view. Only the save-location picker
   * uses it today: `s` opens it, ↑/↓ move, enter confirms, esc cancels. It is
   * deliberately part of the state machine rather than a nested ui.select()
   * overlay — the navigator already owns keyboard focus, and stacking a second
   * dialog on top of a focused custom component fights it for input.
   */
  prompt:
    | {
        kind: "saveLocation";
        ...
        /** Destinations that already hold this name, so the picker can flag an overwrite. */
...
function leftPhaseRow(
  ...
  const name = truncateToWidth(p.title, nameRoom, Ellipsis.Unicode, false);
...
function rightAgentRow(
  ...
  if (modelRoom < 0) {
    ...
    nameOut = truncateToWidth(a.label, nameRoom, Ellipsis.Unicode, false);
  } else {
    modelOut = truncateToWidth(model, modelRoom, Ellipsis.Unicode, false);
...
function topTitleSegment(title: string, innerW: number, leading: boolean, theme: ThemeLike): string {
  ...
  if (visibleWidth(label) > innerW - fixed) {
    labelOut = truncateToWidth(label, Math.max(0, innerW - fixed), Ellipsis.Unicode, false);
  }
...
function renderNavigatorFrame(
  ...
  if (state.kind === "runs") {
    ...
    if (w) {
      ...
      body.push(dim("Location: ") + `${w.location} · ${asText(w.path)}`);
  ...
  if (state.prompt?.kind === "saveLocation") {
    const prompt = state.prompt;
    lines.push("", theme.fg("accent", theme.bold(`Save /${prompt.name} to:`)));
    prompt.options.forEach((option, i) => {
      const selected = i === prompt.cursor;
      // An existing copy at this destination is replaced, not merged — say so
      // before the user commits, not after.
      const overwrite = prompt.existing.includes(option.location) ? "  (overwrite)" : "";
      const row = `${selected ? "›" : " "} ${option.label}  ${option.display}${overwrite}`;
      lines.push(selected ? theme.fg("accent", theme.bold(row)) : dim(row));
    });
  }
...
function twoPaneHeader(
  ...
  const nameText = truncateToWidth(name, width, Ellipsis.Unicode, false);
  ...
  if (rightW >= width) {
    ...
    line1 = theme.fg("dim", truncateToWidth(rightRaw, width, Ellipsis.Unicode, false));
  } else {
    const availL = width - rightW - gap;
    const leftText = availL > 0 ? truncateToWidth(status, availL, Ellipsis.Unicode, false) : "";
...
function footerHint(state: NavigatorState, model: NavigatorModel, theme: ThemeLike): string {
  if (state.prompt) return theme.fg("dim", ["↑/↓ select", "enter save here", "esc cancel"].join(" · "));
  const parts: string[] = [];
  switch (state.kind) {
    case "detail":
      if (state.pagerOpen) {
        ...
        parts.push("enter open pager", "t tail", "s save", "esc back");
      }
      break;
    case "savedDetail":
    ...
    default:
      // phases / agents: still scoped to one run, so `s` saves that run's script.
      parts.push("↑/↓ select", "enter open", "s save", "esc back", "q quit");
  }
...
import type { OverlayAnchor } from "@oh-my-pi/pi-tui";
...
export function openWorkflowNavigator(
  ...
  return ui.custom<void>(
    (tui: TUI, theme: Theme, _keybindings, done: (r: undefined) => void) => {
      ...
      // Live per-agent usage ticks at most ~1/s per running agent, so a wide
      // fan-out can still land several events per second. Coalesce them into a
      // single trailing redraw — every view that shows tokens (run list, phases,
      // agents) reads the same snapshot, so one repaint refreshes all of them.
      let usageRenderTimer: ReturnType<typeof setTimeout> | undefined;
      const onAgentUsage = () => {
        if (usageRenderTimer) return;
        usageRenderTimer = setTimeout(() => {
          usageRenderTimer = undefined;
          rerender();
        }, 250);
        (usageRenderTimer as { unref?: () => void }).unref?.();
      };
      manager.on("agentUsage", onAgentUsage);

      const cleanup = () => {
        for (const ev of events) manager.off(ev, onEvent);
        manager.off("agentHistory", onAgentHistory);
        manager.off("agentUsage", onAgentUsage);
        if (usageRenderTimer) clearTimeout(usageRenderTimer);
        usageRenderTimer = undefined;
      ...
      /**
       * Commit the open save-location choice. Kept next to the picker so the
       * write path (storage.save + slash-command registration + notify) exists
       * exactly once, and errors surface as a notification instead of leaving
       * the prompt stuck open.
       */
      const commitSaveLocation = () => {
        const prompt = state.prompt;
        if (prompt?.kind !== "saveLocation") return;
        state.prompt = undefined;
        const storage = opts.storage;
        const run = manager.listRuns().find((r) => r.runId === prompt.runId);
        if (!storage || !run?.script) {
          ui.notify("Saving is no longer available for this run", "warning");
          return;
        }
        const location = prompt.options[prompt.cursor]?.location ?? "project";
        let saved: ReturnType<WorkflowStorage["save"]>;
        try {
          saved = storage.save(
            { name: prompt.name, description: run.workflowName, script: run.script, location },
            location,
          );
        } catch (error) {
        ...
        const verb = prompt.existing.includes(location) ? "Updated" : "Saved";
        ui.notify(`${verb} /${prompt.name} → ${saved.path}`, "info");
      };

      const act = (data: string) => {
        const key = parseKey(data);
        // The save-location picker is modal: it owns every key while open, so a
        // stray j/x can't stop a run behind the dialog.
        if (state.prompt) {
          const count = state.prompt.options.length;
          if (key === "up" || key === "k") state.prompt.cursor = (state.prompt.cursor - 1 + count) % count;
          else if (key === "down" || key === "j") state.prompt.cursor = (state.prompt.cursor + 1) % count;
          else if (key === "enter" || key === "return") commitSaveLocation();
          else if (key === "escape" || key === "esc" || key === "q") state.prompt = undefined;
          rerender();
          return;
        }
        const itemKind = state.kind === "runs" ? state.itemKindAt(model, state.cursor) : undefined;
        const action = keyToAction(key, state.kind, itemKind);
          ...
          switch (action.type) {
            ...
            case "save": {
              ...
              if (!run?.script) {
                ...
                // Don't write yet: open the destination picker and let the
                // confirm branch below do the save.
                ...
                state.prompt = {
                  ...
                  name,
                  cursor: 0,
                  options: saveLocationOptions(opts.cwd ?? process.cwd()),
                  existing: opts.storage.locationsOf(name),
                };
              }
              break;
            }
