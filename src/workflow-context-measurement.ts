...
export interface WorkflowContextMeasurement {
  formatVersion: 3;
  encoding: "utf8";
  sources: ["src/workflow-tool.ts", "skills/workflow-authoring", "package.json#omp.skills"];
  surfaces: {
    permanentWorkflowPrompt: ByteSurface;
    providerVisibleWorkflowToolDefinition: ByteSurface;
    /**
     * Every skill this package registers (package.json's `omp.skills` — read
 ...
 * `omp.skills` array rather than hardcoded.
 */
function registeredSkillRoots(root: string): string[] {
  const manifest = JSON.parse(readFileSync(join(root, "package.json"), "utf8")) as {
    omp?: { skills?: unknown };
  };
  const skills = manifest.omp?.skills;
  if (!Array.isArray(skills) || skills.length === 0 || !skills.every((s) => typeof s === "string")) {
    throw new Error("package.json must declare a non-empty omp.skills array of skill root paths");
  }
  return skills;
}
  ...
  return {
    ...
    surfaces: {
      permanentWorkflowPrompt: {
        serialization: "UTF-8 bytes of LF-joined OMP prompt lines",
        bytes: promptBytes,
      },
      ...
      registeredSkillsDiscovery: {
        serialization:
          "sum of UTF-8 bytes of normalized OMP skill XML (name + description + location) across every root in package.json's omp.skills",
