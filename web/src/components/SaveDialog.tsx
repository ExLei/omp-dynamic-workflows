  ...
  return (
    ...
    >
      <div className="w-[560px] rounded-lg border border-ink-600 bg-ink-800 shadow-2xl">
        <div className="border-b border-ink-600 px-4 py-2 text-xs tracking-wider text-ink-300 uppercase">
          保存工作流
        </div>
        <div className="space-y-3 p-4">
          <label className="block">
            <span className="mb-1 block text-[10px] tracking-wider text-ink-300 uppercase">
              命令名(注册为 /名称)
            </span>
            <input
              autoFocus
              ...
              className="w-full rounded border border-ink-600 bg-ink-950 px-2 py-1 text-xs outline-none focus:border-accent"
            />
            {!nameValid && <span className="text-[10px] text-bad">名称不能为空,且不能包含 / 或 \</span>}
          </label>

          <label className="block">
            <span className="mb-1 block text-[10px] tracking-wider text-ink-300 uppercase">描述</span>
            <input
          ...
          <div>
            <span className="mb-1 block text-[10px] tracking-wider text-ink-300 uppercase">保存位置</span>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-2">
                        ...
                        {overwrite && (
                          <span className="rounded bg-busy/15 px-1 text-[9px] text-busy">已存在 · 覆盖</span>
                        )}
                        {option.location === "project" && (
                          <span className="ml-auto text-[9px] text-ink-300">优先级高,可随仓库提交</span>
                        )}
                      </span>
                      <span className="mt-0.5 block truncate text-[10px] text-ink-300">{option.dir}</span>
                    </span>
                  </button>
                );
              })}
