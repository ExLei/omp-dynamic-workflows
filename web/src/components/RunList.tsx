...
export function RunList() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <Panel title={`运行 · ${runs.length}`} className="min-h-0 flex-1">
        <ul className="space-y-1 p-2">
          {runs.length === 0 && <li className="px-1 text-xs text-ink-300">暂无运行</li>}
          {runs.map((run) => (
            <li key={run.runId}>
              <button
                type="button"
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-auto text-[10px] text-ink-300">{run.status}</span>
                </div>
                ...
                {run.currentPhase && (
                  <div className="truncate text-[10px] text-accent/80">▸ {run.currentPhase}</div>
                )}
              </button>
            </li>
          ))}
      ...
      <Panel title="已保存 / 内置" className="max-h-[40%] min-h-0 border-t border-ink-600">
        <ul className="space-y-1 p-2">
          {saved.map((entry) => (
            <li key={`saved-${entry.name}`}>
              ...
              >
                ...
                <span className="ml-auto text-[10px] text-ink-300">{entry.location}</span>
              </button>
            </li>
          ))}
          {builtins.map((entry) => (
            <li key={`builtin-${entry.name}`}>
              ...
              >
                ...
                <span className="truncate text-xs">/{entry.name}</span>
                <span className="ml-auto text-[10px] text-ink-300">内置</span>
              </button>
            </li>
          ))}
        </ul>
